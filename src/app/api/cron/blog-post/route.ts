...
- 文量: 1500〜2500文字
- 内容: 「なぜ」と「どうすれば」を必ず両方書く。具体的・実践的に
- Careoへの自然な言及を記事中に1〜2箇所（無理な押し売りは不要）
- HTML形式で返す。使用可能なタグ: <h2> <h3> <p> <ul> <ol> <li> <strong> <em> <blockquote>
- <html><body><head>タグは不要。記事本文のHTMLのみ
- 記事末尾に必ずCareoへのCTA段落を入れる（リンク: <a href="https://careoai.jp/signup" class="blog-cta-link">無料で始める</a>）
- タクミのグロース視点: 「読んだ人が今すぐ行動したくなる」終わり方にする`;
...
{"title":"記事タイトル","description":"メタディスクリプション（120〜160文字）","tags":["タグ1","タグ2","タグ3"],"reading_time_min":読了時間分数}
...
JSONの後に改行2つ置いて、記事本文のHTMLを書いてください。`;
  ...
  // コードブロック（```json ... ``` or ``` ... ```）を除去してからJSONを抽出
  ...
  let meta: { title: string; description: string; tags: string[]; reading_time_min: number };
  try {
    meta = JSON.parse(jsonMatch[0]);
  } catch {
    return NextResponse.json({ error: "JSON invalid", raw: jsonMatch[0] }, { status: 500 });
  }

  // JSON部分を除いた残りが本文HTML
  const afterJson = raw.indexOf(jsonMatch[0]) + jsonMatch[0].length;
  const body = raw.slice(afterJson).replace(/^```[\s\S]*?\n/, "").replace(/```\s*$/, "").trim();
