// Edge Runtimeを使用（Vercel Hobby planでも30秒タイムアウト。Serverlessは10秒）
...
export async function POST(req: Request) {
  try {
    ...
    const monthsLeft = (gradYear - now.getFullYear()) * 12 + (3 - now.getMonth()); // 3月卒業想定
...
scoreは整数（50=普通/75=良好/90=優秀）。`;
    ...
    if (!anthropicRes.ok) {
      ...
      return Response.json({ error: `Anthropic ${anthropicRes.status}: ${errText.slice(0, 300)}` }, { status: 500 });
    }
