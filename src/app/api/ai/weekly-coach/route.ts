...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompt = `あなたは新卒採用コンサルティングファームで面接官育成・評価設計に関わってきたプロのキャリアコーチです。「採用側の評価軸」と「候補者の感情マネジメント」の両面から、この就活生の先週の振り返りと今週のアドバイスをJSONのみで返してください。

評価の視点：
- 面接の感情（緊張/疲れ/手応え）と結果の相関から読み取れる適性や相性
- 1週間の活動量・質から見える「戦略的な一週間だったか」
- 感情と結果にズレがある場合はそこを重点的に言語化
