    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900 dark:text-gray-100">AI企業研究</h2>
        </div>
      ...
      {result && (
        <div className="space-y-5">
          <div>
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300">{result.business}</p>
          </div>
          <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">競合企業</h3>
            <div className="flex flex-wrap gap-2">
              {result.competitors.map((c, i) => (
                <span key={i} className="px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full text-xs">{c}</span>
              ))}
            </div>
          </div>
          <div>
